...
class Game:
    ...
    def start(self):
        processes = [  # создать по процессу на каждого бегуна
            Process(target=runner.run, args=(self._map, ))
            for runner in self._runners
        ]
        for process in processes:
            process.start()  # запустить гонку

        print('Game started')

        for process in processes:
            process.join()  # дождаться всех бегунов на финише

        print('Game finished')


def main():
    print('Program with processes')
